...
/*function job_card_date(){
	new JsDatePick({
		useMode:2,
		target:"job_card_date",
		dateFormat:"%d/%m/%y"
	});	
}
*/
...
/*function checkJobCard(){
	...
	ajaxRequest.onreadystatechange = function(){
		  if(ajaxRequest.readyState == 4 && ajaxRequest.status==200){
			   ...
			   document.getElementById('job_card_date').value=arr[0];
			   document.getElementById('job_card').value=arr[1];
			  
		  }
		};
	
	var	data=Project_select+columnSeperator+visit_store_name;
...
function submitJobCard(){
	...
	//alert(jobCardDate+'    '+jobCard+'  '+Project_select+'   '+visit_store_name);
	...
	Project_select+columnSeperator+visit_store_name;
...
function insertMeaurementSheet(Project_elementID){
 //alert("insert measurement sheet");
	var visit_store_name=document.getElementById('visit_store_name').value;	
	...
	//alert("1");
	var element_status=document.getElementById('e_status_id').value;
	...
	columnSeperator+unit_h_w+columnSeperator+depth+columnSeperator+unit_d+columnSeperator+quantity+columnSeperator+element_status+columnSeperator+componentId+columnSeperator+material_id;
...
/*function heightDropDown(){
	...
	ajaxRequest.onreadystatechange = function(){
	  if(ajaxRequest.readyState == 4 && ajaxRequest.status==200){
		   var result = ajaxRequest.responseText;
		   result = result.trim();
	alert("RESULT height drop down --->"+result);
...
function measurementSheetDisplay(){
	...
	ajaxRequest.open("GET", 'SelectMeasurementData.jsp?visit_store_name='+visit_store_name+'&Project_select='+Project_select,true);	
	ajaxRequest.send(null);
	
}
function checkJobCard(){
	//alert("check job card");
	//var Project_select=document.getElementById('Project_select').value;
	var visit_store_name=document.getElementById('visit_store_name').value;
/*	if(Project_select==''){
		alert("Project Can't Empty !!");
	...
	ajaxRequest.onreadystatechange = function(){
		  if(ajaxRequest.readyState == 4 && ajaxRequest.status==200){
			...
			//  alert("RESULT checkJobCard --->"+result);
			   ...
			   var arr=result.split("#&#");
			   //alert(result);
			   document.getElementById('jc_date').value=arr[1].substr(0,10);
			   document.getElementById('jc_number').value=arr[0];
				   ...
				   document.getElementById('jc_date').value='';
...
function getElementStatus(){
	...
	ajaxRequest.onreadystatechange = function(){
	  if(ajaxRequest.readyState == 4 && ajaxRequest.status==200){
		   ...
		   result = result.trim();
		 //  alert("RESULT region --->"+result);		 
		   document.getElementById('e_status_id').innerHTML=result;
		   document.getElementById('status_id_u').innerHTML=result;
		   
		   var eStatus = document.getElementById('e_status_id');
...
function update_measurement_sheet(){
	//	alert('update_measurement_sheet');
		...
		var surfaceNo=document.getElementById('surface_no_u').value.trim();
		if(surfaceNo==""){
			alert("PLEASE ENTER SURFACE NO.");
		return false;
		}
		var surfaceDetail=document.getElementById('surface_detail_u').value.trim();
		if(surfaceDetail==""){
			surfaceDetail="empty";
		}
		...
		//alert(" element_status_v=="+element_status_v);
