...
<%
	...
	out.println("<select id=\"update_country_select\" style=\"width:180px\" onchange=\"getRegion('updateOperation',1)\"><option value=\"select\">select</option>");		
					...
					<table id="region_detail_table" cellspacing="0">
						...
						<%
							...
							if (ajaxFlag.equals("delete_region")) {
								...
								status = scr.deleteRegion(outData);
								out.println(status);
								/*
								if (status == 0) {
							...
							if (ajaxFlag.equals("update_region")) {
								...
								status = scr.updateRegion(outData);
								out.println(status);
								/*
								if (status == 0) {
