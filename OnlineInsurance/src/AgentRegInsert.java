...
public class AgentRegInsert extends HttpServlet
{
        ...
        throws ServletException, IOException
    {
        ...
        try
        {
            ...
            rs1 = st.executeQuery((new StringBuilder("select branchmgrid from branchmgr where branchmgrname='")).append(mbname).append("'").toString());
            if(rs1.next())
            {
            ...
            rs = st.executeQuery("select max(agentid) from agents");
            if(rs.next())
            {
                bid = rs.getInt(1);
                if(bid == 0)
            ...
            pstmt = con.prepareStatement("insert into agents values(?,?,?,?,?,?,?,?,?,?,?)");
            pstmt.setInt(1, bid);
