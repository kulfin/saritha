...
public class PremiumTX extends HttpServlet
{
        ...
        throws ServletException, IOException
    {
        ...
        try
        {
            ...
            rs = st.executeQuery("select max(custpremiumtxid) from custpremiumtx");
            ...
            pstmt = con.prepareStatement("insert into custpremiumtx values(?,?,?,?)");
            pstmt.setInt(1, pid);
            pstmt.setDate(4, d2);
            pstmt.setInt(2, cpid);
            pstmt.setInt(3, pamt);
            ...
            rs1 = st.executeQuery((new StringBuilder("select * from custpolicies where custpolicyid='")).append(cpid).append("' ").toString());
            ...
            for(rs2 = st.executeQuery((new StringBuilder("select * from agentcommtx where agentid=")).append(agid).append(" ").toString()); rs2.next();)
            ...
            pstmt1 = con.prepareStatement("insert into agentcommtx values(?,?,?,?,?)");
            pstmt1.setInt(1, pid);
